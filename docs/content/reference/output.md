...
describes what users get. Pick a format with `-o`, or let liepin choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
liepin <command> -o table   # aligned columns for reading
liepin <command> -o jsonl   # one JSON object per line, for piping
liepin <command> -o json    # a single JSON array
liepin <command> -o csv     # spreadsheet friendly
liepin <command> -o tsv     # tab-separated
liepin <command> -o url     # just the URL column
liepin <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
liepin <command> --fields id,title,url
```
...
```bash
liepin <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
liepin <command>            # a table, because this is a terminal
liepin <command> | wc -l    # JSONL, because this is a pipe
```
